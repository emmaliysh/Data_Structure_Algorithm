...
public class ExternalChainingHashMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        ExternalChainingMapEntry toBeAdded = new ExternalChainingMapEntry(key, value); 

        if (table[index] == null){
            table[index] = toBeAdded; 

        } else {
            ExternalChainingMapEntry current = table[index];
            while(current != null){
                if (current.getKey().equals(key)){
                    toReturn = (V) current.getValue();
                    current.setValue(value);
                    return toReturn;
                } 
                current = current.getNext();
            } 
            ExternalChainingMapEntry newHeadToAdd = new ExternalChainingMapEntry(key, value, table[index]);

            table[index] = newHeadToAdd;       
        }
    ...
    public V remove(K key) {
        ...
        ExternalChainingMapEntry current = table[keyIndex];

        if (current == null){
            throw new NoSuchElementException("the key is not in the map");
        } else{
            // case 1: key is the only element here
            if (current.getKey().equals(key)){
                toReturn = (V) current.getValue();
                table[keyIndex] = current.getNext();
                size --;
                return toReturn;
            } 

            // case 2: key is not the first element 
            while(current.getNext() != null){
                if (current.getNext().getKey().equals(key)){
                    toReturn = (V) current.getNext().getValue();
    ...
    private void resizeBackingTable(int length) {
        ...
        // copy the table's data to another one
        ExternalChainingMapEntry<K, V>[] originalTable = table;
        int originalSize = size;

        // easier to call put method for later copying
        ...
        for (int i = 0; i < originalTable.length; i++){
            if (originalTable[i] != null){
                ExternalChainingMapEntry current = originalTable[i];
                put((K)current.getKey(), (V)current.getValue());
